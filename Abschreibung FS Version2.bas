...
'
    ...
    Range("M2").Select
    ActiveCell.FormulaR1C1 = "=SUM(1*IFERROR(SEARCH({""FS"";""Foodsharing""},RC[-5]),0))"
    ...
    For r = 1 To FL
        ...
        If Not IsEmpty(FL_name) Then
            ...
            Else
                Sheets("Tabelle1").Select
                ActiveSheet.Range("$A$1:$L$" & All_Rows).AutoFilter Field:=13
                ...
                Else
                    Sheets("Tabelle1").Select
                    ActiveSheet.Range("$A$1:$M$" & All_Rows).AutoFilter Field:=13, Criteria1:="1", Operator:=xlFilterValues
